...
import { createUserWithEmailAndPassword, GoogleAuthProvider, TwitterAuthProvider, signInWithPopup } from "firebase/auth";
     ...
     try {
       ...
       console.error(`Erro no login/cadastro com ${providerName}:`, error);
        if (error.code === 'auth/account-exists-with-different-credential') {
          ...
          setError(`Login bem-sucedido. Parece que você já tinha uma conta com este email via ${providerName}.`);
          // O listener do App.jsx deve redirecionar de qualquer forma.
        } else if (error.code === 'auth/popup-closed-by-user') {
           setError(`Cadastro com ${providerName} cancelado.`);
       } else {
          setError(`Falha ao cadastrar com ${providerName}. Verifique o console.`);
        }
     } finally {
         setSocialLoading(prev => ({ ...prev, [providerName]: false }));
     }
    ...
    <div className={styles.authContainer}>
      ...
      {/* Botões Sociais */}
       <div className={styles.socialLoginButtons}>
         ...
         >
           {socialLoading.google ? 'Aguarde...' : <><FaGoogle /> Continuar com Google</>}
         </button>
         <button
           type="button"
           ...
           disabled={socialLoading.google || socialLoading.twitter || loading}
         >
           {socialLoading.twitter ? 'Aguarde...' : <><FaTwitter /> Continuar com Twitter</>}
         </button>
         {/* Placeholders */}
         <button type="button" disabled className={`${styles.socialButton} ${styles.disabledButton}`}>Twitch (Em breve)</button>
         <button type="button" disabled className={`${styles.socialButton} ${styles.disabledButton}`}>Instagram (Em breve)</button>
       </div>
